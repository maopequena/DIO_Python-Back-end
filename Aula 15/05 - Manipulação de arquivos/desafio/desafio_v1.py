...
class PessoaFisica(Cliente):
    ...
    def __repr__(self) -> str:  # representação do objeto em modo texto
...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        # desafio extra: aqui podemos percorrer as listas de args e kwargs com for para melhorar a aparência
        # desafio extra: também podemos salvar as transações em um arquivo CSV, para isso, em menu, carregar os arquivos CSV em "clientes" e "contas", ao invés da lista vazia
        log_path = ROOT_PATH / "log.txt"

        with open(log_path, "a", encoding="utf-8") as log:
            log.write(
                f"\n[{data_hora}]\tFunção: {func.__name__.upper()}"
                f"\nCom os seguintes argumentos: {args}, {kwargs}. Retorna {resultado}\n"
            )
